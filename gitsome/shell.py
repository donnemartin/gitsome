...
from gitsome.environ import xonshrc_context
from gitsome.tools import XonshError
...
class Shell(object):
    ...
    def __init__(self, ctx=None, shell_type=None, **kwargs):
        ...
        if env['SHELL_TYPE'] == 'prompt_toolkit':
            from gitsome.prompt_toolkit_shell import PromptToolkitShell
            self.shell = PromptToolkitShell(execer=self.execer,
                                            ctx=self.ctx,
                                            **kwargs)
        elif env['SHELL_TYPE'] == 'readline':
            from gitsome.readline_shell import ReadlineShell
